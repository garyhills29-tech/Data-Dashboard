...
if "transactions" not in st.session_state:
    st.session_state.transactions = {
        "checking": [{"date": "2025-11-18", "desc": "Opening Balance", "amount":  +12340.50}],
        "savings": [{"date": "2025-11-18", "desc": "Opening Balance", "amount": +14911.32}],
...
<style>
    ...
    .glass-card {background: rgba(255,255,255,0.98); color:#000; border-radius:16px; padding:30px; box-shadow:0 10px 40px rgba(0,0,0,0.3); margin:20px 0; cursor:pointer;}
...
irs_seal_base64 = "data:image/png;base64,"
...
# ========================= PAGES =========================
def login_page():
    st.markdown("<div class='truist-header'><div style='font-size:100px'>🏦</div><h1 style='color:white'>Welcome to Truist Online Banking</h1><p style='color:#b700'>Educational Demo</p></div>", unsafe_allow_html=True)
...
def accounts():
    ...
    if st.button("Premier Checking ••••2847 → View History", use_container_width=True):
    ...
    if st.button("High-Yield Savings ••••5901 → View History", use_container_width=True):
...
def view_account_history():
    account = st.session_state.viewing
    balance = st.session_state.checking if account == "checking" else st.session_state.savings
    st.markdown(f"<h1 style='text-align:center;color:#ffb700'>{account.title()} History</h1>", unsafe_allow_html=True)
    st.metric("Current Balance", f"${balance:,.2f}")
    if account in st.session_state.transactions and st.session_state.transactions[account]:
        df = pd.DataFrame(st.session_state.transactions[account])
...
def crypto_wallet():
    ...
    for symbol, amt in st.session_state.crypto.items():
        if amt > 0:
            ...
            if st.button(f"{symbol} — {amt:,.6f} coins → View History", use_container_width=True):
...
if not st.session_state.authenticated:
    ...
    if "viewing" in st.session_state:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Total Balance", f"${st.session_state.checking + st.session_state.savings:,.2f}")
        with col2:
